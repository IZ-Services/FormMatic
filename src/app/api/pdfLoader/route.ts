...
export async function POST(request: NextRequest) {
  const { formData, pdfUrl } = await request.json();
	...
	async function modifyReg101Pdf(fileUrl: string, formData: FormData) {
		...
		console.log("Fields in the PDF form:", JSON.stringify(allFields.map(field => field.getName()), null, 2));
	...
	async function modifyReg227Pdf(fileUrl : string, formData: FormData) {
		...
		const fullName1 = constructFullName(formData.lastName1, formData.firstName1, formData.middleName1);
		...
		const fullName2 = constructFullName(formData.lastName2, formData.firstName2, formData.middleName2);
